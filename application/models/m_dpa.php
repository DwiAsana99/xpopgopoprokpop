...
class M_dpa extends CI_Model
{
	...
	function import_from_rka($id_skpd, $ta){
		...
		$query="SELECT
					...
					kd_program,
					kd_kegiatan,
					tx_rka_prog_keg.nama_prog_or_keg,
					lokasi,
					penanggung_jawab,
...
FROM tx_dpa_indikator_prog_keg WHERE target > 0)) AS keg ON keg.parent=pro.id
	...
	function get_cetak_rencana_program($id, $bulan=NULL){
		...
		$result = $this->db->query("SELECT ROUND(a.capaian/a.target*100, 2) AS for_capaian, a.* FROM tx_dpa_rencana_aksi a WHERE id_dpa_prog_keg = '".$id."' ".$where."  ORDER BY bulan,get_date");
		return $result;
	}
	...
	function get_kegiatan_for_221_new($ta, $idK){
		$query = $this->db->query("SELECT kd_urusan AS kode_urusan, kd_bidang AS kode_bidang, kd_program AS kode_program, kd_kegiatan AS kode_kegiatan
			...
			,parent, lokasi, id AS as_id, tahun AS as_tahun, id_skpd AS as_id_skpd
			,(SELECT SUM(subtotal) FROM tx_dpa_belanja_kegiatan WHERE id_keg = as_id) AS nominal
			,(SELECT id FROM tx_dpa_prog_keg WHERE kd_urusan = kode_urusan AND kd_bidang = kode_bidang AND kd_program = kode_program AND kd_kegiatan = kode_kegiatan AND tahun = as_tahun-1 AND id_skpd = as_id_skpd) AS as_id_lalu
			,(SELECT SUM(subtotal) FROM tx_dpa_belanja_kegiatan WHERE id_keg = as_id_lalu) AS nominal_thnlalu
			,(SELECT id FROM tx_dpa_prog_keg WHERE kd_urusan = kode_urusan AND kd_bidang = kode_bidang AND kd_program = kode_program AND kd_kegiatan = kode_kegiatan AND tahun = as_tahun+1 AND id_skpd = as_id_skpd) AS as_id_depan
			,(SELECT SUM(subtotal) FROM tx_dpa_belanja_kegiatan WHERE id_keg = as_id_depan) AS nominal_thndpn
			FROM tx_dpa_prog_keg
			WHERE id = '$idK'");
		return $query->row();
	}

	function get_belanja_for_221_new($ta, $idK){
	...
	function sumber_dana_rekap($tahun, $id_skpd){
		return $this->db->query("SELECT * FROM (
			...
			ref.sumber_dana,
			SUM( IF( ref.tahun = '$tahun', ref.subtotal, 0) ) AS 'tahun1'
			FROM (SELECT id_keg, 
	...
	function get_kegiatan_for_211_new($ta, $idK){
		$query = $this->db->query("SELECT kd_urusan AS kode_urusan, kd_bidang AS kode_bidang, kd_program AS kode_program, kd_kegiatan AS kode_kegiatan
			...
			,(SELECT Ket_Program FROM m_program WHERE Kd_Urusan = kode_urusan  AND Kd_Bidang = kode_bidang AND Kd_Prog = kode_program) AS nama_program
			,(SELECT Ket_Kegiatan FROM m_kegiatan WHERE Kd_Urusan = kode_urusan AND Kd_Bidang = kode_bidang AND Kd_Prog = kode_program AND Kd_Keg = kode_kegiatan) AS nama_kegiatan
			,SUM(`nominal_1`+`nominal_2`+`nominal_3`+`nominal_4`+`nominal_5`+`nominal_6`+`nominal_7`+`nominal_8`+`nominal_9`+`nominal_10`+`nominal_11`+`nominal_12`) AS nominal,
			parent, lokasi, id_skpd
			FROM tx_dpa_prog_keg
			WHERE id = '$idK'");
		return $query->row();
	}
	...
	function copy_belanja_kegiatan($keg_dari, $keg_tujuan){
		...
		$this->db->query("
			INSERT INTO  tx_dpa_belanja_kegiatan (
			tahun,
			kode_urusan,
			kode_bidang,
			kode_program,
		...
		// 	FROM tx_dpa_belanja_kegiatan AS ref
		// 	WHERE id_keg = '$keg_tujuan'")->row();
		// $this->db->query("UPDATE tx_dpa_prog_keg SET nominal = '".$total->total_skr."', nominal_thndpn = '".$total->total_dpn."'
		// 	WHERE id = '$keg_tujuan'");
