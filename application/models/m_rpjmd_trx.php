...
// exit();
	...
	function get_urusan_all(){
		...
		// (SELECT SUM(nominal_1) FROM t_renstra_prog_keg WHERE kd_urusan = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_1 > 0)) AS nom1,
		// (SELECT SUM(nominal_2) FROM t_renstra_prog_keg WHERE kd_urusan = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_2 > 0)) AS nom2,
		// (SELECT SUM(nominal_3) FROM t_renstra_prog_keg WHERE kd_urusan = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_3 > 0)) AS nom3,
		// (SELECT SUM(nominal_4) FROM t_renstra_prog_keg WHERE kd_urusan = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_4 > 0)) AS nom4,
		// (SELECT SUM(nominal_5) FROM t_renstra_prog_keg WHERE kd_urusan = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_5 > 0)) AS nom5
		// FROM t_renstra_prog_keg
		// WHERE id_skpd > 0 
		// GROUP BY kd_urusan, nama";

		$query = "SELECT pro.kd_urusan AS kd,
		(SELECT nm_urusan FROM m_urusan WHERE m_urusan.kd_urusan = pro.kd_urusan) AS nama,
		SUM(keg.nominal_1) AS nom1, SUM(keg.nominal_2) AS nom2, SUM(keg.nominal_3) AS nom3, SUM(keg.nominal_4) AS nom4, SUM(keg.nominal_5) AS nom5 
		FROM 
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=1 AND id_skpd > 0) AS pro
		INNER JOIN
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=2 AND id_skpd > 0) AS keg ON keg.parent=pro.id
		WHERE pro.id_skpd = keg.id_skpd
	...
	function get_bidang_urusan($kd_urusan){
		...
		// (SELECT SUM(nominal_1) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_1 > 0)) AS nom1,
		// (SELECT SUM(nominal_2) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_2 > 0)) AS nom2,
		// (SELECT SUM(nominal_3) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_3 > 0)) AS nom3,
		// (SELECT SUM(nominal_4) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_4 > 0)) AS nom4,
		// (SELECT SUM(nominal_5) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = kd AND id_skpd > 0 AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_5 > 0)) AS nom5
		// FROM t_renstra_prog_keg
		// WHERE kd_urusan = '$kd_urusan' AND id_skpd > 0
		// GROUP BY kd_urusan, kd_bidang";

		$query = "SELECT pro.kd_bidang AS kd,
		(SELECT nm_bidang FROM m_bidang WHERE m_bidang.kd_urusan = pro.kd_urusan AND m_bidang.kd_bidang = pro.kd_bidang) AS nama,
		SUM(keg.nominal_1) AS nom1, SUM(keg.nominal_2) AS nom2, SUM(keg.nominal_3) AS nom3, SUM(keg.nominal_4) AS nom4, SUM(keg.nominal_5) AS nom5 
		FROM 
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=1 AND id_skpd > 0 AND kd_urusan = '".$kd_urusan."') AS pro
		INNER JOIN
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=2 AND id_skpd > 0 AND kd_urusan = '".$kd_urusan."') AS keg ON keg.parent=pro.id
		WHERE pro.id_skpd = keg.id_skpd
	...
	function get_skpd_bidang_urusan($kd_urusan, $kd_bidang){
		...
		// (SELECT SUM(nominal_1) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = '$kd_bidang' AND id_skpd > 0 AND id_skpd = kd AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_1 > 0)) AS nom1,
		// (SELECT SUM(nominal_2) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = '$kd_bidang' AND id_skpd > 0 AND id_skpd = kd AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_2 > 0)) AS nom2,
		// (SELECT SUM(nominal_3) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = '$kd_bidang' AND id_skpd > 0 AND id_skpd = kd AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_3 > 0)) AS nom3,
		// (SELECT SUM(nominal_4) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = '$kd_bidang' AND id_skpd > 0 AND id_skpd = kd AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_4 > 0)) AS nom4,
		// (SELECT SUM(nominal_5) FROM t_renstra_prog_keg WHERE kd_urusan = '$kd_urusan' AND kd_bidang = '$kd_bidang' AND id_skpd > 0 AND id_skpd = kd AND
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_5 > 0)) AS nom5
		// FROM t_renstra_prog_keg
		...
		$query = "SELECT pro.id_skpd AS kd,
		(SELECT nama_skpd FROM m_skpd WHERE m_skpd.id_skpd = pro.id_skpd) AS nama, 
		(SELECT kode_skpd FROM m_skpd WHERE m_skpd.id_skpd = pro.id_skpd) AS kode_skpd,
		SUM(keg.nominal_1) AS nom1, SUM(keg.nominal_2) AS nom2, SUM(keg.nominal_3) AS nom3, SUM(keg.nominal_4) AS nom4, SUM(keg.nominal_5) AS nom5 
		FROM 
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=1 AND id_skpd > 0 AND kd_urusan = '".$kd_urusan."' AND kd_bidang = '".$kd_bidang."') AS pro
		INNER JOIN
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=2 AND id_skpd > 0 AND kd_urusan = '".$kd_urusan."' AND kd_bidang = '".$kd_bidang."') AS keg ON keg.parent=pro.id
		WHERE pro.id_skpd = keg.id_skpd
		GROUP BY pro.id_skpd, nama
		ORDER BY kode_skpd";
	...
	function get_sum_urusan_per_skpd($id_skpd, $group=FALSE){
		...
		if ($group) {
			...
			// $where2 = " AND kd_urusan = kd_ur AND kd_bidang = kd_bid";
			$where3 = " GROUP BY kd_urusan, kd_bidang";
		}
		...
		// $query = "SELECT crot.kd_urusan, crot.kd_bidang, crot.kd_urusan as kd_ur, crot.kd_bidang as kd_bid,
		// (SELECT SUM(nominal_1) FROM t_renstra_prog_keg WHERE id_skpd = '$id_skpd' AND 
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_1 > 0) 
		// ".$where2.") as crots1,
		// (SELECT SUM(nominal_2) FROM t_renstra_prog_keg WHERE id_skpd = '$id_skpd' AND 
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_2 > 0)
		// ".$where2.") as crots2,
		// (SELECT SUM(nominal_3) FROM t_renstra_prog_keg WHERE id_skpd = '$id_skpd' AND 
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_3 > 0)
		// ".$where2.") as crots3,
		// (SELECT SUM(nominal_4) FROM t_renstra_prog_keg WHERE id_skpd = '$id_skpd' AND 
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_4 > 0)
		// ".$where2.") as crots4,
		// (SELECT SUM(nominal_5) FROM t_renstra_prog_keg WHERE id_skpd = '$id_skpd' AND 
		// id IN (SELECT id_prog_keg FROM t_renstra_indikator_prog_keg WHERE target_5 > 0)
		// ".$where2.") as crots5,
		...
		$query = "SELECT pro.kd_urusan, pro.kd_bidang, pro.kd_urusan as kd_ur, pro.kd_bidang as kd_bid,
		(SELECT nm_urusan FROM m_urusan WHERE kd_urusan = pro.`kd_urusan`) AS urusan,
		(SELECT nm_bidang FROM m_bidang WHERE kd_urusan = pro.`kd_urusan` AND kd_bidang = pro.`kd_bidang`) AS bidang,
		SUM(keg.nominal_1) AS crots1, SUM(keg.nominal_2) AS crots2, SUM(keg.nominal_3) AS crots3, SUM(keg.nominal_4) AS crots4, SUM(keg.nominal_5) AS crots5 
		FROM 
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=1 AND id_skpd > 0) AS pro
		INNER JOIN
		(SELECT * FROM t_renstra_prog_keg WHERE is_prog_or_keg=2 AND id_skpd > 0) AS keg ON keg.parent=pro.id
		WHERE pro.id_skpd = keg.id_skpd AND pro.id_skpd = '$id_skpd' ".$where3;
