...
class M_rka_perubahan extends CI_Model
{
	...
	var $table_indikator_program_perubahan = 'tx_rka_indikator_prog_keg_perubahan';
	var $is_program = 1;
	var $is_kegiatan = 2;
	...
	function count_jendela_kontrol($id_skpd,$ta){
		if($this->session->userdata("id_skpd") > 100){
			...
			$search = "AND tx_rka_prog_keg_perubahan.id_skpd in (SELECT id_skpd FROM m_asisten_sekda WHERE id_asisten = '$id_skpd')";
		}else {
			$kode_unit = $this->session->userdata("id_skpd");
			if ($id_skpd == $kode_unit) {
				$search = "AND tx_rka_prog_keg_perubahan.id_skpd in (SELECT id_skpd FROM m_skpd WHERE kode_unit = '$id_skpd')";
			}else {
				$search = "AND (tx_rka_prog_keg_perubahan.id_skpd = '$id_skpd' OR tx_rka_prog_keg_perubahan.id_skpd = '$kode_unit')";
			}
		}
		$query = "SELECT
	...
	function import_from_renja($id_skpd, $ta){
		...
		$query="SELECT
					$ta AS tahun,
					t_ppas_prog_keg_perubahan.id AS id_renja,
					is_prog_or_keg,
					kd_urusan,
					kd_bidang,
					kd_program,
					...
					penanggung_jawab,
					t_ppas_prog_keg_perubahan.id_skpd,
					nominal,
					nominal_thndpn,
					t_ppas_prog_keg_perubahan.id_prog_rpjmd
				FROM t_ppas_prog_keg_perubahan
					WHERE t_ppas_prog_keg_perubahan.is_prog_or_keg=1 AND
					tahun=$ta AND
					t_ppas_prog_keg_perubahan.id_skpd in (SELECT id_skpd FROM m_skpd WHERE kode_unit = ?)";
		...
		foreach ($rka_baru as $row) {
			...
			SELECT ?, indikator,indikator, satuan_target,satuan_target, status_indikator, status_indikator, kategori_indikator, kategori_indikator, target, target_thndpn , t_ppas_indikator_prog_keg_perubahan.id
				FROM t_ppas_indikator_prog_keg_perubahan WHERE id_prog_keg=?";
			...
			$query="SELECT
					...
					? AS parent
				FROM t_ppas_prog_keg_perubahan
				WHERE t_ppas_prog_keg_perubahan.is_prog_or_keg=2 AND
					tahun=$ta AND t_ppas_prog_keg_perubahan.parent=?
					AND t_ppas_prog_keg_perubahan.id_skpd in (SELECT id_skpd FROM m_skpd WHERE kode_unit = ?)";
			...
			foreach ($kegiatan_rka_baru as $row1) {
				...
				$query = "INSERT INTO tx_rka_indikator_prog_keg_perubahan(id_prog_keg, indikator, indikator_thndpn, satuan_target, satuan_target_thndpn, status_indikator, status_indikator_thndpn, kategori_indikator, kategori_indikator_thndpn, target, target_thndpn, id_indikator_renja)
											SELECT ?, indikator,indikator, satuan_target,satuan_target, status_indikator, status_indikator, kategori_indikator, kategori_indikator, target, target_thndpn, t_ppas_indikator_prog_keg_perubahan.id
											FROM t_ppas_indikator_prog_keg_perubahan WHERE id_prog_keg=?";
				...
				//belanja kegiatan
				$query2 = "
							INSERT INTO `tx_rka_belanja_kegiatan_perubahan`
							(
							`id_renja`,
							`tahun`,
							`kode_urusan`,
	...
	function get_indikator_prog_keg_vdee($id, $return_result=TRUE, $satuan=FALSE){
		...
		$this->db->from('t_ppas_indikator_prog_keg_perubahan');
	...
	function edit_kegiatan_skpd($data, $id_kegiatan, $indikator, $id_indikator_kegiatan, $satuan_target,$status_indikator, $kategori_indikator, $target, $target_thndpn){
		...
		// $this->db->query("delete from tx_rka_belanja_kegiatan_perubahan where id_keg = $id_kegiatan ");
		...
		// $this->db->insert_batch('tx_rka_belanja_kegiatan_perubahan', $datatahun1_batch);

		// $this->db->insert_batch('tx_rka_belanja_kegiatan_perubahan', $datatahun2_batch);
		...
		function get_program_skpd_4_cetak_vdeeygatas($id_skpd,$tahun,$kd_urusan,$kd_bidang)
    {
		...
		function get_program_skpd_4_cetak_vdee($id_skpd,$tahun,$kd_urusan,$kd_bidang){
			...
			$query = "SELECT * from (
						...
						union
						select tahun,kd_urusan,kd_bidang,kd_program,kd_kegiatan,id_skpd from tx_rka_prog_keg_perubahan where id_skpd = '".$id_skpd."' and tahun = '".$tahun."' and kd_kegiatan is not null
						) k
						left join t_ppas_prog_keg_perubahan r
						on k.tahun = r.tahun
						...
						left join tx_rka_prog_keg_perubahan as rp
						on k.tahun = rp.tahun
						...
						) AS kendali WHERE (kendali.nomrka > 0 OR kendali.nomrka_perubahan > 0)";
			//$data = array($id_skpd,$tahun,$kd_urusan,$kd_bidang);
			$result = $this->db->query($query);
			return $result->result();
		}
    ...
    function get_kegiatan_skpd_4_cetak($id_skpd,$tahun,$kd_urusan,$kd_bidang,$kd_program){
		$query = "SELECT
					...
					SELECT tahun,
						...
						kd_program,
						kd_kegiatan,
						id_skpd FROM tx_rka_prog_keg_perubahan
						WHERE id_skpd = ".$id_skpd."
						AND tahun = ".$tahun."
						AND kd_kegiatan IS NOT NULL
					) k
					LEFT JOIN t_ppas_prog_keg_perubahan AS r
					ON k.tahun = r.tahun
					AND k.kd_urusan = r.kd_urusan
					AND k.kd_bidang = r.kd_bidang
					AND k.kd_program = r.kd_program
	...
	function get_rka_belanja_per_tahun221($ta,$ta_tahun,$id_kegiatan){
		//------- query by deesudi
			...
			$query = $this->db->query("SELECT id ,tahun,
							...
							SELECT id FROM `tx_rka_prog_keg_perubahan` WHERE Kd_Urusan = kode_urusan  AND Kd_Bidang = kode_bidang AND kd_program = kode_program AND is_prog_or_keg ='1'
							) AS id_program,(
							SELECT nominal FROM `tx_rka_prog_keg_perubahan` WHERE id = '$id_kegiatan'
							) AS nominal_tahun,
...
function get_indikator_keluaran($ta, $idK){
			$query = $this->db->query("SELECT * FROM `tx_rka_indikator_prog_keg_perubahan` WHERE   `id_prog_keg` = '$idK'");
		return $query->result();
	}

	function get_indikator_capaian( $idP){
			$query = $this->db->query("SELECT * FROM `tx_rka_indikator_prog_keg_perubahan` WHERE   `id_prog_keg` = '$idP'");
		return $query->result();
	}
	function get_nominal_rka( $idK, $is_tahun_sekarang){
		...
		if ($is_tahun_sekarang == '1') {
			$query = $this->db->query("SELECT  nominal_thndpn ,
									CASE WHEN
									(SELECT
										 nominal FROM tx_rka_prog_keg_perubahan WHERE  id = '$idK' AND tahun  =  tahun-1
									) IS NULL THEN 0 ELSE
									(SELECT
										 nominal FROM tx_rka_prog_keg_perubahan WHERE  id = '$idK' AND tahun  =  tahun-1
									) END AS nominal_min , nominal
									FROM `tx_rka_prog_keg_perubahan` WHERE id = '$idK' ");
		}else{
	...
	function get_kegiatan($id_kegiatan, $tahun=NULL, $is_tahun=NULL, $not_in=NULL){
				...
				$query = "SELECT id ,tahun,
								...
								uraian_belanja, detil_uraian_belanja, volume, satuan, nominal_satuan, subtotal, is_tahun_sekarang, id_status_rka, id_keg
								FROM tx_rka_belanja_kegiatan_perubahan
	...
	function add_belanja_kegiatan($data, $id_belanja){
		if (!empty($id_belanja)) {
			...
			$this->db->update('tx_rka_belanja_kegiatan_perubahan', $data);
		}else{
			$this->db->insert('tx_rka_belanja_kegiatan_perubahan', $data);
			$id_baru = $this->db->insert_id();
			$this->db->query('INSERT INTO tx_rka_belanja_kegiatan_perubahan(`id_renja`,`tahun`,`kode_urusan`,`kode_bidang`,`kode_program`,`kode_kegiatan`,`kode_sumber_dana`,
			...
			`is_tahun_sekarang`,`id_status_rka`,`id_keg`) 
				SELECT `id_renja`,`tahun`,`kode_urusan`,`kode_bidang`,`kode_program`,`kode_kegiatan`,`kode_sumber_dana`,
			`kode_jenis_belanja`,`kode_kategori_belanja`,`kode_sub_kategori_belanja`,`kode_belanja`,
			`uraian_belanja`,`detil_uraian_belanja`, 0, "-", 0, 0,`created_date`,
			1,`id_status_rka`,`id_keg` FROM tx_rka_belanja_kegiatan_perubahan WHERE `id` = "'.$id_baru.'"');
		}
	}

	function delete_one_kegiatan($id){
		$belanja = $this->db->query('SELECT * FROM tx_rka_belanja_kegiatan_perubahan WHERE id = "'.$id.'"')->row();

		$this->db->query('DELETE FROM tx_rka_belanja_kegiatan_perubahan WHERE `tahun` = "'.$belanja->tahun.'" AND 
		 ...
		 `id_status_rka` = "'.$belanja->id_status_rka.'"');
		$this->db->query('DELETE FROM tx_rka_belanja_kegiatan_perubahan WHERE id = "'.$id.'"');
	}
