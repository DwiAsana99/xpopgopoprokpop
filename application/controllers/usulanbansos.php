...
class usulanbansos extends CI_Controller
{
	...
	function index(){
		...
		$this->auth->restrict();
		redirect('usulanbansos/usulanhibahbansos/hibah');
	...
	function cetak_data_usulan($status, $id_jenishibah=NULL){
		$this->auth->restrict();
		if ($status == 1) {
			$data['status_ng'] = 'HIBAH';
			$jenis_usulan = 'Hibah-';
		}else{
			$data['status_ng'] = 'BANSOS';
			$jenis_usulan = 'Bansos-';
		}

		$jenis = array('' => 'SEMUA', '1' => 'UANG', '2' => 'BARANG', '3' => 'JASA');

		$data['jenis_hibah'] = $jenis[$id_jenishibah];
		$tahun = $this->m_settings->get_tahun_anggaran();
		$data['tahun'] = $tahun;
		$data['usulan'] = $this->m_usulan_bansos->get_all_usulan(NULL, NULL, NULL, NULL, NULL, NULL, $status, $id_jenishibah);

		$result = $this->load->view('usulanbansos/cetak/cetak_all_usulan', $data, TRUE);

		// print_r($result);exit();
	 	$this->create_pdf->load_ng($result, 'Data-Usulan-'.$jenis_usulan.date("d-m-Y_H:i:s"), 'Letter-L','');
	}

	function usulanhibahbansos($status, $id_jenishibah=NULL){
		$this->auth->restrict();

		if ($status == 'hibah') {
			$data['is_hibah_ng'] = 1;
			$cb_jenis = array('' => 'Semua', '1' => 'Uang', '2' => 'Barang');
		}else{
			$data['is_hibah_ng'] = 0;
			$cb_jenis = array('' => 'Semua', '1' => 'Uang', '2' => 'Barang', '3' => 'Jasa');
		}
		$data['status_ng'] = $status;
		$data['cb_jenis'] = form_dropdown('cb_jenis', $cb_jenis, $id_jenishibah, ' class="chosen-select" id="cb_jenis"');

        $data['url_add_data'] = site_url('usulanbansos/edit_data/'.$status);
        $data['url_load_data'] = site_url('usulanbansos/load_data');
        $data['url_delete_data'] = site_url('usulanbansos/delete_data');
        $data['url_edit_data'] = site_url('usulanbansos/edit_data/'.$status);
        $data['url_save_data'] = site_url('usulanbansos/save_data');
		$data['url_show_gallery'] = site_url('usulanbansos/show_gallery');
	...
	function persetujuanusulanbansos($status, $id_jenishibah=NULL){
		$this->auth->restrict();
		...
		$data['status_ng'] = $status;
		$data['id_jenishibah_ng'] = $id_jenishibah;
		$data['cb_jenis'] = form_dropdown('cb_jenis', $cb_jenis, $id_jenishibah, ' class="chosen-select" id="cb_jenis"');
				...
				// if($this->session->userdata('id_skpd') == '1'){
					...
					$result = $this->m_usulan_bansos->get_one_usulan_persetujuan($search, $start, $length, $order["0"], $order_arr, NULL, $data['is_hibah_ng'], $id_jenishibah);
...
//exit();
	...
	function evaluasiusulanbansos($status, $id_jenishibah=NULL){
		$this->auth->restrict();
	...
	private function cetak_func($cetak=FALSE, $is_hibah_ng=NULL, $id_jenishibah=NULL){
			...
			$order_arr = array('id_musrenbang', 'nama_skpd','nama_kec','nama_desa', 'jenis_pekerjaan');
			$usulanbansos['usulanbansos'] = $this->m_usulan_bansos->get_all_usulan_persetujuan($search, $start, $length, $order["0"], $order_arr, NULL, $is_hibah_ng, $id_jenishibah);

			$result = $this->load->view("usulanbansos/usulan_view_persetujuan_cetak", $usulanbansos, TRUE);
			return $result;
		}

	function cetakpersetujuanusulanbansos($is_hibah_ng=NULL, $id_jenishibah=NULL){
        $search = $this->input->post("search");
		$start = $this->input->post("start");
		$length = $this->input->post("length");
		$order = $this->input->post("order");
		...
		$order_arr = array('id_musrenbang', 'nama_skpd','nama_kec','nama_desa', 'jenis_pekerjaan');
		$usulanbansos['usulan'] = $this->m_usulan_bansos->get_all_usulan_persetujuan($search, $start, $length, $order["0"], $order_arr, NULL, $is_hibah_ng, $id_jenishibah);
		...
		// $data['cetak'] = $this->cetak_func(TRUE, $is_hibah_ng, $id_jenishibah);
		...
		if ($status == 1) {
			$usulanbansos['status_ng'] = 'HIBAH';
			$jenis_usulan = 'Hibah-';
		}else{
			$usulanbansos['status_ng'] = 'BANSOS';
			$jenis_usulan = 'Bansos-';
		}
		$usulanbansos['tahun'] = $this->m_settings->get_tahun_anggaran();
		$result = $this->load->view('usulanbansos/cetak/cetak_persetujuan_usulan', $usulanbansos, TRUE);
	 	$this->create_pdf->load_ng($result, 'Data-Rekapitulasi-Usulan-'.$jenis_usulan.date("d-m-Y_H:i:s"), 'Letter-L','');
	...
	function load_data_persetujuan(){
		...
		$length = $this->input->post("length");
		$order = $this->input->post("order");
		$is_hibah_ng = $this->input->post('is_hibah');
		$id_jenishibah = $this->input->post('cb_jenis');
		...
		// if ($this->session->userdata('id_skpd')=='1') {
			...
			$order_arr = array('id_musrenbang', 'nama_skpd','nama_kec','nama_desa', 'jenis_pekerjaan');
			$usulanbansos = $this->m_usulan_bansos->get_all_usulan_persetujuan($search, $start, $length, $order["0"], $order_arr, NULL, $is_hibah_ng, $id_jenishibah);
	//		console.log($usulanbansos);
	// print_r($this->db->last_query());

			$alldata = $this->m_usulan_bansos->count_all_usulan_persetujuan($search, NULL, $is_hibah_ng, $id_jenishibah);

				$no=0;
				$total_nilai_usulan = 0;
				$total_nominal_anggaran = 0;
				$total_nominal_rekomendasi = 0;
			foreach ($usulanbansos as $row) {
				$no++;
				$mp_filefiles = $this->m_usulan_bansos->get_file(explode( ',', $row->filerekomendasi), TRUE)->row();
				// $preview_action = '<a href="javascript:void(0)" onclick="preview_modal(\''. base_url().$mp_filefiles->location .'\')">'. $mp_filefiles->file .'</a>';
				$preview_action = '<a href="javascript:void(0)" onclick="preview_modal(\''. base_url().$mp_filefiles->location .'\', \''. $mp_filefiles->file .'\')" class="icon2-note" title="Lihat Dokumen"/>';
				...
				if(is_null($row->nominal_setuju)){
					$NominalAnggaran = '<input type="hidden" value="'.$row->id_usulan_bansos.'" name ="id['.$no.']"><input type="text" oninput="kup(this)" value="'.Formatting::currency($row->nominal_rekomendasi,2).'" id="det_uraian['.$no.']" name="anggaran['.$no.']" class="common jos" />';
			$total_nominal_anggaran += $row->nominal_rekomendasi;
				}else{
			...
			$total_nominal_anggaran += $row->nominal_setuju;
				};
				...
				$total_nilai_usulan += $row->jumlah_dana;
				$total_nominal_rekomendasi += $row->nominal_rekomendasi;
				...
				$data[] = array(
						...
						//		$row->nama_desa,
							...
							//	$row->lokasi,
								...
								$statusRKPD ,
								$NominalAnggaran,
								Formatting::currency($row->nominal_rekomendasi,2),
								$rekomendasi,
								$row->tglrekomendasi,
								$preview_action
								);
		...
		$data[] = array(
			...
			'',
			'<b>'.Formatting::currency($total_nilai_usulan, 2).'</b>',
			'',
			'',
			'<b>'.Formatting::currency($total_nominal_anggaran, 2).'</b>',
			'<b>'.Formatting::currency($total_nominal_rekomendasi, 2).'</b>',
			'',
			'',
			''
		);
    ...
    $search = $this->input->post("search");
		...
		$order_arr = array('id_musrenbang', 'nama_skpd','nama_kec','nama_desa', 'jenis_pekerjaan');
		$usulanbansos = $this->m_usulan_bansos->get_all_usulan_evaluasi($search, $start, $length, $order["0"], $order_arr, NULL, $is_hibah_ng, $id_jenishibah);
		//echo $this->db->last_query();
		//exit();
		$alldata = $this->m_usulan_bansos->count_all_usulan_evaluasi($search, NULL, $is_hibah_ng, $id_jenishibah);

		$data = array();
		$no=0;
		foreach ($usulanbansos as $row) {
			$no++;
			// $mp_filefiles = base_url().$this->m_usulan_bansos->get_file(explode( ',', $row->file), TRUE)->row()->location;
			// $preview_action = '<a href="javascript:void(0)" onclick="preview_modal(\''. $mp_filefiles .'\')" class="icon-search" title="Lihat Usulan"/>';
			$edit_action = '<a href="javascript:void(0)" onclick="edit_usulan_table('. $row->id_usulan_bansos .')" class="icon2-note" title="Konfirmasi Usulan Hibah/Bansos"/>';
	...
	$data = array();
	 ...
	 $alldata=0;
		if ($this->session->userdata('id_skpd')=='6') {
			...
			$order_arr = array('id_musrenbang', 'nama_skpd','nama_kec','nama_desa', 'jenis_pekerjaan');
			$usulanbansos = $this->m_usulan_bansos->get_all_usulan($search, $start, $length, $order["0"], $order_arr, NULL, $is_hibah_ng, $id_jenishibah);
			// print_r($this->db->last_query()) ;
			// exit();
			$alldata = $this->m_usulan_bansos->count_all_usulan($search, NULL, $is_hibah_ng, $id_jenishibah);
    ...
    function edit_data($status=NULL, $id=NULL){
	...
	function edit_data_evaluasi($id=NULL){
		...
		//	echo $this->db->last_query();
			...
			$data['nominal_rekomendasi'] = $result->nominal_rekomendasi;
    ...
    function save(){
        ...
        $data_post = array(
    		...
    		'fasilitator'			=> $this->input->post('id_groups'),
				...
				'nama_pengusul_ng' => $this->input->post('nama_pengusul_ng'),
		...
		if (!empty($cekusulan->file)) {
    		...
    		if (!empty($hapus)) {
    			foreach ($hapus as $row) {
			    	...
			    	$var_hapus = $this->m_usulan_bansos->get_one_file($row);
    ...
    function savepersetujuan($is_hibah_ng){
		$date=date("Y-m-d");
        $time=date("H:i:s");
        $this->auth->restrict();
		//$id = $this->input->post('id_usulanbansos');
       ...
       for ($i =1; $i<= $max;$i++){
        ...
        redirect('usulanbansos/persetujuanusulanbansos/'.$is_hibah_ng);
		//var_dump($cekbank);
		//print_r ($id_cek);
    }
    ...
    function save_evaluasi($is_hibah_ng){
		$date=date("Y-m-d");
        $time=date("H:i:s");
        $this->auth->restrict();
		$id = $this->input->post('id_usulanbansos');
        ...
        $data_post = array(
    		...
    		'nominal_rekomendasi'			=> $this->input->post('nominal_rekomendasi'),
...
//exit();
       ...
       redirect('usulanbansos/evaluasiusulanbansos/'.$is_hibah_ng);
		//var_dump($cekbank);
	...
	function persetujuanusulanbansosrekap($is_hibah = NULL){
		$this->auth->restrict();
		$data['url_load_data'] = site_url('usulanbansos/load_data_persetujuan_rekap/'.$is_hibah);
		// $data['url_edit_data'] = site_url('usulanbansos/edit_data_persetujuan');
		if ($is_hibah == 1) {
			$data['jenis_us'] = 'Hibah';
		}else{
			$data['jenis_us'] = 'Bansos';
		}
		$data['is_hibah'] = $is_hibah;
		...
		$this->template->load('template','usulanbansos/usulan_view_persetujuan_rekap',$data);
	}

	function load_data_persetujuan_rekap($is_hibah = NULL, $kec=NULL, $desa=NULL, $pilihan=NULL, $statusRekomendasi=NULL){
        $search = $this->input->post("search");
		$start = $this->input->post("start");
		$length = $this->input->post("length");
		$order = $this->input->post("order");
		...
		$where = "";
		if (!empty($kec) && $kec != 'all') {
			$where .= " AND id_kecamatan = ".$kec;
		}
		if (!empty($desa) && $desa != 'all') {
			$where .= " AND id_desa = ".$desa;
		}
		if ($pilihan != NULL && $pilihan != 'all') {
			$where .= " AND id_pilihanrenja = ".$pilihan;
		}
		if ($statusRekomendasi == '0') {
			$where .= " AND (norekomendasi IS NULL OR norekomendasi = '')";
		}elseif ($statusRekomendasi == '1') {
	...
	// exit();
			$order_arr = array('id_musrenbang', 'nama_skpd','nama_kec','nama_desa', 'jenis_pekerjaan');
			$usulanbansos = $this->m_usulan_bansos->get_all_usulan_persetujuan_rekap($search, $start, $length, $order["0"], $order_arr, $is_hibah, NULL, $where);
	//		console.log($usulanbansos);
// print_r($this->db->last_query());
			...
			foreach ($usulanbansos as $row) {
				...
				$NominalAnggaran = Formatting::currency($row->nominal_setuju,2);
    ...
    function cetak_data_persetujuan_rekap($is_hibah, $kec, $desa, $pilihan, $statusRekomendasi){
    	$this->auth->restrict();
		...
		// $protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
		// $gambar = $this->m_template_cetak->get_value("GAMBAR");
		// $data['logo'] = str_replace("src=\"","height=\"90px\" src=\"".$protocol.$_SERVER['HTTP_HOST'],$gambar);
		$data['tahun'] = $this->m_settings->get_tahun_anggaran();
		$data['usulan'] = $this->m_usulan_bansos->get_all_usulan_persetujuan_rekap(NULL, NULL, NULL, NULL, NULL, $is_hibah, NULL, $where);


		$result = $this->load->view('usulanbansos/cetak/cetak_rekapitulasi_usulan', $data, TRUE);
		// print_r($result);exit();
	 	$this->create_pdf->load_ng($result, 'Data-Rekapitulasi-Usulan-'.$jenis_usulan.date("d-m-Y_H:i:s"), 'Letter-L','');
    }
