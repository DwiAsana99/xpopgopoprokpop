...
class Dpa extends CI_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model(array('m_dpa','m_skpd','m_lov','m_urusan', 'm_bidang', 'm_program', 'm_kegiatan', 'm_jenis_belanja', 'm_kategori_belanja', 'm_subkategori_belanja', 'm_kode_belanja',
															'm_template_cetak', 'm_settings', 'm_kendali', 'm_rpjmd_trx'));
...
FROM tx_dpa_indikator_prog_keg WHERE target > 0)) AS keg ON keg.parent=pro.id
	...
	function preview_dpa(){
		...
		$skpd = $this->m_dpa->get_one_dpa_skpd($id_skpd_unit, TRUE);
		if (!empty($skpd)) {
	...
	// 	$data['data_keg'] = $this->db->query('select * from tx_dpa_prog_keg where id = '.$idK)->row();
	// 	$data['tahun1'] =$this->m_dpa->get_dpa_belanja_per_tahun221($ta, $idK);

	// 	$data['keluaran'] =$this->m_dpa->get_indikator_keluaran($ta, $idK);
	// 	$data['capaian'] =$this->m_dpa->get_indikator_capaian($data['data_keg']->parent);
...
// print_r($data);
	...
	function belanja_kegiatan_hapus(){
		...
		$data['edit'] = $this->m_dpa->get_one_belanja($id);
	...
	private function cetak_func221($cetak=FALSE, $ta, $idK, $id_skpd=null){
		...
		$data['capaian'] =$this->m_dpa->get_indikator_capaian($data['kegiatan']->parent);
		$data['belanja'] = $this->m_dpa->get_belanja_for_221_new($ta, $idK);
		// print_r($this->db->last_query());  
	...
	function preview_cetak_kegiatan_for_veri($idK) {
		$ta = $this->m_settings->get_tahun_anggaran();
		$keg = $this->m_dpa->get_kegiatan_for_211_new($ta, $idK);
