...
// exit;
	...
	function cru_program_skpd(){
		...
		if (!empty($id)) {
			...
			$data['id_prog_prioritas_edit'] = $id_prog_prioritas_edit;
			// print_r($result);
			// exit;
	...
	function cru_kegiatan_skpd(){
		...
		$prog_induk = $this->m_renja_trx->get_one_program($id_program);
		$prog_prio_induk = $prog_induk->id_prioritas_daerah;

		$id_prog_prioritas = array("0" => "Non Prioritas");
		foreach ($this->m_prioritas_pembangunan_rkpd->get_prog_prioritas_by_skpd($id_skpd, $ta, '2', $prog_prio_induk)->result() as $row) {
			$id_prog_prioritas[$row->id] = $row->id_prog_or_keg;
		}
		...
		$data['id_prog_prioritas'] = form_dropdown('id_prioritas_daerah', $id_prog_prioritas, $id_prog_prioritas_edit, 'data-placeholder="Pilih Kegiatan Prioritas" class="common chosen-select" id="id_prioritas_daerah"');
	...
	function save_kegiatan(){
		...
		$data = array(
		  ...
		  'is_prog_or_keg' => $this->input->post('is_prog_or_keg'),
			...
			'id_prioritas_daerah' => $this->input->post('id_prioritas_daerah')
		);
...
// print_r($data);
	...
	function preview_renja(){
		...
		$skpd = $this->m_renja_trx->get_one_renja_skpd($id_skpd_unit, TRUE);
		if (!empty($skpd)) {

			$data = $this->cetak_skpd_func_new($id_skpd, $ta, $id_skpd_unit);
	...
	private function cetak_skpd_func_new($id_skpd,$ta,$id_skpd_unit){
		//$proses = $this->m_renja_trx->count_jendela_kontrol($id_skpd);
		$data['renja_type'] = "RENJA";
...
FROM t_renja_indikator_prog_keg WHERE target > 0)) AS keg ON keg.parent=pro.id
	...
	// 	$data['data_keg'] = $this->db->query('select * from t_renja_prog_keg where id = '.$idK)->row();
	...
	// 	$data['capaian'] =$this->m_renja_trx->get_indikator_capaian($data['data_keg']->parent);
	...
	private function cetak_func221($cetak=FALSE, $ta, $is_thn_sekarang, $idK) {
		...
		$data['capaian'] =$this->m_renja_trx->get_indikator_capaian($data['kegiatan']->parent);
		$data['belanja'] = $this->m_renja_trx->get_belanja_for_221_new($ta, $is_thn_sekarang, $idK);
