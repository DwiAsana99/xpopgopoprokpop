...
// print_r($data);
	...
	function rekap_sumber_dana($cetak=FALSE){
		...
		$data['tahun'] = $th;
		$data['id_skpd'] = $id_skpd;
		$data['data1'] = $this->m_ppas->sumber_dana_rekap($th, $id_skpd)->result();

		if (!$cetak) {
			$this->template->load('template','ppas/cetak/cetak_sumber_dana', $data);
		}else{

			$protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
			$header = $this->m_template_cetak->get_value("GAMBAR");
			$data['logo'] = str_replace("src=\"","height=\"90px\" src=\"".$protocol.$_SERVER['HTTP_HOST'],$header);
			$data['qr'] = $this->ciqrcode->generateQRcode("sirenbangda", 'Rekap Sumber Dana PPAS '. date("d-m-Y H-i-s"), 2);
