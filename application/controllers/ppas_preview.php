<?php  
    ...
    class ppas_preview extends CI_Controller {
        ...
        public function index()
        {
		    ...
		    $data = $this->get_urusan($ta, TRUE);
		    $this->template->load('template', 'ppas/preview_ppas', $data);
        }

        private function get_urusan($ta, $ppas=FALSE){
			//$proses = $this->m_renja_trx->count_jendela_kontrol($id_skpd);
			$pro_in = "";
			if ($ppas) {
				$pro_in = "AND pro.id IN (SELECT id_prog_keg FROM t_ppas_indikator_prog_keg WHERE id_prog_keg = pro.id AND target > 0)";
			}
			$data['ppas_type'] = "PPAS";
			...
			$data2['urusan'] = $this->db->query("
				...
				INNER JOIN
					(SELECT * FROM t_ppas_prog_keg WHERE is_prog_or_keg=2 AND id_skpd > 0 AND id IN (SELECT id_prog_keg FROM t_ppas_indikator_prog_keg WHERE target > 0)) AS keg ON keg.parent=pro.id
				WHERE
					keg.tahun= ".$ta." ".$pro_in." 
			...
			")->result();
			$data2['ta'] = $ta;
			$data['ppas'] = $this->load->view('ppas/cetak/program_kegiatan_all', $data2, TRUE);
			return $data;
		}

		private function get_urusan_khusus($ta, $rkpd = FALSE){
			//$proses = $this->m_renja_trx->count_jendela_kontrol($id_skpd);
			$pro_in = "";
			if ($rkpd) {
				$pro_in = "AND pro.id IN (SELECT id_prog_keg FROM t_ppas_indikator_prog_keg WHERE id_prog_keg = pro.id AND target > 0)";
			...
			")->result();
			$data2['ta'] = $ta;
			$data['ppas'] = $this->load->view('ppas/cetak/tanpa_program_kegiatan', $data2, TRUE);
			return $data;
		}

		function ppas_tanpa_program()
		{
			$this->auth->restrict();
			$id_group = $this->session->userdata('id_group');
			$ta = $this->session->userdata('t_anggaran_aktif');
			...
			$data = $this->get_urusan_khusus($ta, TRUE);

			$this->template->load('template', 'ppas/preview_ppas_tanpa_program', $data);
		}
		...
		function ppas_skpd(){
			...
			$this->template->load('template', 'ppas/preview_ppas_skpd', $data);
		}

		function get_data_ppas_skpd()
		{
			...
			$data['urusan'] = $this->m_ppas->get_urusan_skpd($ta, $id_skpd);
			
			$this->load->view('ppas/cetak/isi_ppas_skpd', $data);
		}

		function ppas_skpd_tanpa_program()
		{
			$this->auth->restrict();
			$id_group = $this->session->userdata('id_group');
			$ta = $this->session->userdata('t_anggaran_aktif');
			...
			$this->template->load('template', 'ppas/preview_ppas_skpd_tanpa_program', $data);
		}
		...
		private function get_urusan_skpd_khusus($ta)
		{
			...
			$data2['skpd_select'] = $this->db->query("
							...
							")->result();
			$data2['ta'] = $ta;
			$data['ppas'] = $this->load->view('ppas/cetak/skpd_tanpa_program_kegiatan', $data2, TRUE);
			return $data;
		}

		function do_cetak_ppas_skpd_tanpa_program()
		{
			ini_set('memory_limit','-1');
			$ta = $this->session->userdata('t_anggaran_aktif');
				$data = $this->get_urusan_skpd_khusus($ta);
				$data['qr'] = $this->ciqrcode->generateQRcode("sirenbangda", 'RKPD SKPD Tanpa Program Kegiatan'. date("d-m-Y_H-i-s"), 1);
				$html = $this->template->load('template_cetak', 'ppas/cetak/cetak_ppas_skpd_tanpa_program', $data, TRUE);
		...
		function do_export(){	
			...
			foreach ($urusan as $row_urusan) {
				$sum_thn_skrng += $row_urusan->sum_nominal;
				$sum_thn_dpn += $row_urusan->sum_nominal_thndpn;
			
				$this->export_excel->set_row(array($row_urusan->kd_urusan, NULL, NULL, NULL, strtoupper($row_urusan->nama_urusan), NULL, NULL, NULL, Formatting::currency($row_urusan->sum_nominal, 2), NULL,  NULL, Formatting::currency($row_urusan->sum_nominal_thndpn, 2)));			
				...
				foreach ($bidang as $row_bidang) {
					$this->export_excel->set_row(array($row_urusan->kd_urusan, $row_bidang->kd_bidang, NULL, NULL, strtoupper($row_bidang->nama_bidang), NULL, NULL, NULL, Formatting::currency($row_bidang->sum_nominal, 2), NULL, NULL, Formatting::currency($row_bidang->sum_nominal_thndpn, 2)));
					$this->export_excel->merge_cell('E'.$this->export_excel->get_last_row(),'H'.$this->export_excel->get_last_row());
					...
					foreach ($skpd as $row_skpd) {
						...
						foreach ($program as $row) {
							...
							$this->export_excel->set_row(array(
																...
																Formatting::currency($row->sum_nominal, 2),
																$row->catatan,
																$temp[0]->target_thndpn." ".$temp[0]->satuan_target_thndpn,
																Formatting::currency($row->sum_nominal_thndpn, 2),
															)
														);					    
							...
							foreach ($kegiatan as $row) {
								...
								$this->export_excel->set_row(array(
																...
																$row->lokasi, 
																$temp[0]->target." ".$temp[0]->satuan_target,
																Formatting::currency($row->nominal, 2),
																$row->catatan,
																$temp[0]->target_thndpn." ".$temp[0]->satuan_target_thndpn,		    									
																Formatting::currency($row->nominal_thndpn, 2)
															)
														);
			...
			$this->export_excel->filename = "PPAS ".$ta." ";
			...
			$this->export_excel->set_row(array(NULL, NULL, NULL, NULL, 'Total Nominal Tahun Ini', NULL, NULL, NULL, Formatting::currency($sum_thn_skrng, 2), 'Total Nominal Tahun Depan',  NULL, Formatting::currency($sum_thn_skrng, 2)));			
				$this->export_excel->merge_cell('A'.$this->export_excel->get_last_row(),'D'.$this->export_excel->get_last_row());
				$this->export_excel->merge_cell('E'.$this->export_excel->get_last_row(),'H'.$this->export_excel->get_last_row());
				$this->export_excel->merge_cell('J'.$this->export_excel->get_last_row(),'K'.$this->export_excel->get_last_row());
				$this->export_excel->getActiveSheet()->getStyle("A".$this->export_excel->get_last_row().":L".$this->export_excel->get_last_row())->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => 'C0C0C0')), 'font' => array('bold' => true)));
		...
		function do_cetak_ppas_tanpa_program(){
			ini_set('memory_limit','-1');
			$ta = $this->session->userdata('t_anggaran_aktif');
				$data = $this->get_urusan_khusus($ta);
				$data['qr'] = $this->ciqrcode->generateQRcode("sirenbangda", 'PPAS Tanpa Program Kegiatan'. date("d-m-Y_H-i-s"), 1);
				$html = $this->template->load('template_cetak', 'ppas/cetak/cetak_ppas_tanpa_program', $data, TRUE);
	
				$filename = 'PPAS Tanpa Program Kegiatan'. date("d-m-Y_H-i_s") .'.pdf';
				pdf_create($html,$filename,"A4","Landscape");
		}

		function do_cetak_ppas_skpd($id_skpd = NULL, $tahun = NULL)
		{
			ini_set('memory_limit','-1');
			$ta = $this->session->userdata('t_anggaran_aktif');
			...
			$html = $this->template->load('template_cetak', 'ppas/cetak/cetak_ppas_skpd', $data, TRUE);

			$filename = 'PPAS'. date("d-m-Y_H-i_s") .'.pdf';
			pdf_create($html,$filename,"A4","Landscape");
		}

		private function get_rkpd_skpd_cetak($id_skpd,$tahun){
			$skpd_detail = $this->m_skpd->get_one_skpd(array('id_skpd' => $id_skpd));
			$data['ppas_type'] = "PPAS ".strtoupper($skpd_detail->nama_skpd);
			...
			$data['logo'] = str_replace("src=\"","height=\"90px\" src=\"".$protocol.$_SERVER['HTTP_HOST'],$header);
			$data['header'] = $this->m_template_cetak->get_value("HEADER");*/
			$data2['urusan'] = $this->m_ppas->get_urusan_skpd($tahun, $id_skpd);
			$data2['ta'] = $tahun;
			$data2['id_skpd'] = $id_skpd;
			$data['ppas'] = $this->load->view('ppas/cetak/isi_ppas_skpd', $data2, TRUE);
			return $data;
		}
    
    }
    
    /* End of file ppas_preview.php */
