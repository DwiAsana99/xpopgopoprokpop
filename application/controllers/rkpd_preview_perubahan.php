...
class Rkpd_preview_perubahan extends CI_Controller {
	...
	public function __construct(){
        ...
        parent::__construct();
        $this->load->model(array('m_renja_trx_perubahan', 'm_dpa', 'm_skpd', 'm_template_cetak', 'm_lov', 'm_urusan', 'm_bidang', 'm_rkpd',
	...
	private function get_urusan($ta, $rkpd=FALSE){
		//$proses = $this->m_renja_trx->count_jendela_kontrol($id_skpd);
		$pro_in = "";
		if ($rkpd) {
			$pro_in = "AND pro.id IN (SELECT id_prog_keg FROM t_renja_indikator_prog_keg_perubahan WHERE id_prog_keg = pro.id AND target > 0)";
		}
		$data['rkpd_type'] = "RKPD";
...
// 			FROM
// 				(SELECT * FROM t_renja_prog_keg_perubahan WHERE is_prog_or_keg=1 AND id_skpd > 0) AS pro
// 			INNER JOIN
// 				(SELECT * FROM t_renja_prog_keg_perubahan WHERE is_prog_or_keg=2 AND id_skpd > 0 AND id IN (SELECT id_prog_keg 
// FROM t_renja_indikator_prog_keg_perubahan WHERE target > 0)) AS keg ON keg.parent=pro.id
// 			WHERE
// 				keg.tahun= ".$ta." ".$pro_in." 
...
// 		")->result();
	...
	function do_cetak_rkpd(){
		...
		$ta = $this->session->userdata('t_anggaran_aktif');
			...
			$html = $this->template->load('template_cetak', 'renja_perubahan/cetak/cetak_rkpd', $data, TRUE);
	...
	function do_export(){	
		...
		$urusan = $this->db->query("
									...
									SELECT
										...
										SUM(keg.nominal_thndpn) AS sum_nominal_thndpn
									FROM
										(SELECT * FROM t_renja_prog_keg_perubahan WHERE is_prog_or_keg=1) AS pro
									INNER JOIN
										(SELECT * FROM t_renja_prog_keg_perubahan WHERE is_prog_or_keg=2) AS keg ON keg.parent=pro.id
									WHERE
										keg.tahun= ".$ta."
		...
		foreach ($urusan as $row_urusan) {
			...
			foreach ($bidang as $row_bidang) {
				...
				foreach ($skpd as $row_skpd) {
					...
					$program = $this->m_renja_trx_perubahan->get_program_skpd_4_cetak_v2($id_skpd,$ta,$row_urusan->kd_urusan,$row_bidang->kd_bidang);

					foreach ($program as $row) {
					    $result = $this->m_renja_trx_perubahan->get_kegiatan_skpd_4_cetak($row->id);
					    $kegiatan = $result->result();
					    $indikator_program = $this->m_renja_trx_perubahan->get_indikator_prog_keg($row->id, FALSE, TRUE);
	...
	private function get_urusan_khusus($ta, $rkpd = FALSE){
		...
		$pro_in = "";
		if ($rkpd) {
			$pro_in = "AND pro.id IN (SELECT id_prog_keg FROM t_renja_indikator_prog_keg WHERE id_prog_keg = pro.id AND target > 0)";
		}
...
// 		")->result();
	...
	function do_cetak_rkpd_tanpa_program(){
		...
		$ta = $this->session->userdata('t_anggaran_aktif');
			...
			$data['qr'] = $this->ciqrcode->generateQRcode("sirenbangda", 'RKPD Perubahan Tanpa Program Kegiatan'. date("d-m-Y_H-i-s"), 1);
			$html = $this->template->load('template_cetak', 'renja_perubahan/cetak/cetak_rkpd_tanpa_program', $data, TRUE);

			$filename = 'RKPD Perubahan Tanpa Program Kegiatan'. date("d-m-Y_H-i_s") .'.pdf';
			pdf_create($html,$filename,"A4","Landscape");
	}
