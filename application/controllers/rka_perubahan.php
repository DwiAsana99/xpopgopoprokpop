...
class Rka_perubahan extends CI_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model(array('m_jenis_belanja','m_kategori_belanja','m_subkategori_belanja','m_kode_belanja',
				'm_rka_perubahan','m_skpd','m_lov','m_urusan', 'm_bidang', 'm_program', 'm_kegiatan', 'm_template_cetak', 'm_rpjmd_trx'));
	...
	function rka_skpd(){
		...
		if (empty($data['program'])) {
			if ($kode_unit != $id_skpd) {
				redirect('home/kosong/RKA_Perubahan');
			}
		}
	...
	function get_renja(){
		...
		if ($result) {
			$msg = array('success' => '1', 'msg' => 'PPAS Perubahan berhasil diambil.');
			echo json_encode($msg);
		}else{
			$msg = array('success' => '0', 'msg' => 'ERROR! PPAS Perubahan gagal diambil, mohon menghubungi administrator.');
			echo json_encode($msg);
		}
	}
	...
	function save_program_rka(){
		...
		if (!empty($id)) {
			$result = $this->m_rka_perubahan->edit_program_skpd($data, $id, $indikator, $id_indikator_program, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn);
		}else{
			$result = $this->m_rka_perubahan->add_program_skpd($data, $indikator, $satuan_target,$status_indikator, $kategori_indikator, $target, $target_thndpn);
		}
...
// print_r($this->db->last_query());
	...
	function cru_kegiatan_skpd(){
		...
		//------------------------TAHUN SEKARANG
		$data['cb_jenis_belanja_1'] = form_dropdown('cb_jenis_belanja_1', $cb_jenis_belanja, $cb_jenis_belanja_edit, 'data-placeholder="Pilih Jenis Belanja" class="common chosen-select" id="cb_jenis_belanja_1"');
		$data['cb_kategori_belanja_1'] = form_dropdown('cb_kategori_belanja_1', $cb_kategori_belanja, $cb_kategori_belanja_edit, 'data-placeholder="Pilih Kategori Belanja" class="common chosen-select" id="cb_kategori_belanja_1"');
		$data['cb_subkategori_belanja_1'] = form_dropdown('cb_subkategori_belanja_1', $cb_subkategori_belanja, $cb_subkategori_belanja_edit, 'data-placeholder="Pilih Sub Kategori Belanja" class="common chosen-select" id="cb_subkategori_belanja_1"');
		$data['cb_belanja_1'] = form_dropdown('cb_belanja_1', $cb_belanja, $cb_belanja_edit, 'data-placeholder="Pilih Belanja" class="common chosen-select" id="cb_belanja_1"');
		//------------------------TAHUN DEPAN
		$data['cb_jenis_belanja_2'] = form_dropdown('cb_jenis_belanja_2', $cb_jenis_belanja, $cb_jenis_belanja_edit, 'data-placeholder="Pilih Jenis Belanja" class="common chosen-select" id="cb_jenis_belanja_2"');
		$data['cb_kategori_belanja_2'] = form_dropdown('cb_kategori_belanja_2', $cb_kategori_belanja, $cb_kategori_belanja_edit, 'data-placeholder="Pilih Kategori Belanja" class="common chosen-select" id="cb_kategori_belanja_2"');
		$data['cb_subkategori_belanja_2'] = form_dropdown('cb_subkategori_belanja_2', $cb_subkategori_belanja, $cb_subkategori_belanja_edit, 'data-placeholder="Pilih Sub Kategori Belanja" class="common chosen-select" id="cb_subkategori_belanja_2"');
		$data['cb_belanja_2'] = form_dropdown('cb_belanja_2', $cb_belanja, $cb_belanja_edit, 'data-placeholder="Pilih Belanja" class="common chosen-select" id="cb_belanja_2"');

		$data['detil_kegiatan_1'] = $this->m_rka_perubahan->get_rka_belanja_per_tahun($id, '1');
		$data['detil_kegiatan_2'] = $this->m_rka_perubahan->get_rka_belanja_per_tahun($id, '0');
	...
	function save_kegiatan(){
		...
		if (!empty($id)) {
			$result = $this->m_rka_perubahan->edit_kegiatan_skpd($data, $id, $indikator, $id_indikator_kegiatan, $satuan_target,$status_indikator, $kategori_indikator, $target, $target_thndpn);
		}else{
			$result = $this->m_rka_perubahan->add_kegiatan_skpd($data, $indikator, $satuan_target,$status_indikator, $kategori_indikator, $target, $target_thndpn);
		}
	...
	function belanja_kegiatan_lihat($return, $id_kegiatan, $ta, $tahun, $is_tahun, $not=NULL){
		$result = $this->m_rka_perubahan->get_kegiatan($id_kegiatan, $ta, $is_tahun, $not);
// print_r($this->db->last_query());
// print_r($result);
// exit();
		$i = 1;
...
// print_r($data);
// exit();
		$this->m_rka_perubahan->add_belanja_kegiatan($data, $id_belanja);
	...
	function belanja_kegiatan_edit(){
		...
		$data['edit'] = $this->m_rka_perubahan->get_one_belanja($id_belanja);
	...
	function belanja_kegiatan_hapus(){
		...
		$this->m_rka_perubahan->delete_one_kegiatan($id);
	...
	function preview_kegiatan_renja(){
		$id = $this->input->post("id");
		$result = $this->m_rka_perubahan->get_one_kegiatan(NULL, $id, TRUE);
		if (!empty($result)) {
			$data['renja'] = $result;
			$data['indikator_kegiatan'] = $this->m_rka_perubahan->get_indikator_prog_keg_perubahan($result->id, TRUE, TRUE);


			$data['tahun1'] = $this->m_rka_perubahan->get_rka_belanja_per_tahun($id, '1');
			$data['tahun2'] = $this->m_rka_perubahan->get_rka_belanja_per_tahun($id, '0');
			$this->load->view('rka_perubahan/preview', $data);
		}else{
	...
	private function cetak_func221($cetak=FALSE, $ta, $is_tahun, $idK){
		...
		$data['tahun1'] =$this->m_rka_perubahan->get_rka_belanja_per_tahun221($ta, $is_tahun, $idK);

		$data['keluaran'] =$this->m_rka_perubahan->get_indikator_keluaran($ta, $idK);


		$data['capaian'] =$this->m_rka_perubahan->get_indikator_capaian($idK);
		$data['nominal'] =$this->m_rka_perubahan->get_nominal_rka($idK,$ta);
	// print_r($ta);
	// 		exit();
		...
		function preview_periode_221(){
			...
			$this->load->view('rka_perubahan/periode_221',$data);
