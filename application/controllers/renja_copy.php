...
class Renja_copy extends CI_Controller
{
	var $CI = NULL;
	...
	function cru_kegiatan_skpd(){
		...
		$kategori_indikator_thndpn = array("" => "~~ Pilih Kategori Indikator ~~");
		foreach ($this->m_lov->get_kategori_indikator() as $row) {
			$kategori_indikator_thndpn[$row->kode_kategori_indikator]=$row->nama_kategori_indikator;
		}
	...
	function save_kegiatan(){
		...
		if (!empty($id)) {
			$result = $this->m_renja_trx->edit_kegiatan_skpd($data, $id, $indikator, $id_indikator_kegiatan, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn, $dataKegiatan1, $dataKegiatan2);
		}else{
			$result = $this->m_renja_trx->add_kegiatan_skpd($data, $indikator, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn, $dataKegiatan1, $dataKegiatan2);
		}
...
//		$data['header'] = "<p>". strtoupper($skpd_detail->nama_skpd) ."<BR>KABUPATEN KLUNGKUNG, PROVINSI BALI - INDONESIA<BR>".$skpd_detail->alamat."<BR>Telp.".$skpd_detail->telp_skpd."<p>";
	...
	function get_all_data(){
		$query = "SELECT a.kd_program, a.kd_kegiatan, 
			a.nama_prog_or_keg, b.nama_skpd, 
			a.nominal FROM t_renja_prog_keg a
			INNER JOIN m_skpd b
			ON a.id_skpd = b.id_skpd
			WHERE is_prog_or_keg = 2";
