...
class Rka extends CI_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model(array('m_rka','m_skpd','m_lov','m_urusan', 'm_bidang', 'm_program', 'm_kegiatan', 'm_jenis_belanja', 'm_kategori_belanja', 'm_subkategori_belanja', 'm_kode_belanja',
	...
	function get_jendela_kontrol(){
		...
		$data['rka'] = $this->m_rka->get_rka($id_skpd,$ta);
		//$data['renja'] = $this->m_renja_trx->get_one_renja_skpd($id_skpd, TRUE);

		$this->load->view('rka/jendela_kontrol', $data);
	}
	...
	function rka_skpd(){
		...
		$data['id']			= $id;
		$data['program'] = $this->m_rka->get_all_program($id_skpd,$ta);
		$data['id_skpd'] = $id_skpd;
		$data['ta']	= $ta;
		...
		if (empty($data['program'])) {
			if ($kode_unit != $id_skpd) {
				// redirect('home/kosong/RKA');
			}
		}
		$this->template->load('template','rka/view', $data);
	}

	function get_renstra(){
		$id_skpd 	= $this->session->userdata("id_skpd");
		$ta 		= $this->m_settings->get_tahun_anggaran();
		$rka		= $this->m_rka->insert_rka($id_skpd,$ta);
		$result 	= $this->m_rka->import_from_renja($id_skpd,$ta);
		if ($result) {
			$msg = array('success' => '1', 'msg' => 'PPAS berhasil diambil.');
			echo json_encode($msg);
		}else{
			$msg = array('success' => '0', 'msg' => 'ERROR! PPAS gagal diambil, mohon menghubungi administrator.');
			echo json_encode($msg);
		}
	...
	function cru_program_skpd(){
		...
		$this->load->view("rka/cru_program", $data);
	}
	...
	function save_program_rka(){
		...
		if (!empty($id)) {
			$result = $this->m_rka->edit_program_skpd($data, $id, $indikator, $id_indikator_program, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn);
		}else{
			$result = $this->m_rka->add_program_skpd($data, $indikator, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn);
		}
	...
	function delete_program(){
		$this->auth->restrict();
		$id = $this->input->post('id');
		$result = $this->m_rka->delete_program($id);
		if ($result) {
	...
	function cru_kegiatan_skpd(){
		...
		if (!empty($id)) {
			$result = $this->m_rka->get_one_kegiatan($id_program,$id);
			if (empty($result)) {
				echo '<div style="width: 400px;">ERROR! Data tidak ditemukan.</div>';
				return FALSE;
			}
			$data['kegiatan'] = $result;
			$data['indikator_kegiatan'] = $this->m_rka->get_indikator_prog_keg($id, FALSE);
			$kd_kegiatan_edit = $result->kd_kegiatan;

		}
		$data['id_program'] = $id_program;
		$kodefikasi = $this->m_rka->get_info_kodefikasi_program($id_program);
		...
		$data['detil_kegiatan_1'] = $this->m_rka->get_rka_belanja_per_tahun($id, '1');
		$data['detil_kegiatan_2'] = $this->m_rka->get_rka_belanja_per_tahun($id, '0');

		// print_r($data);
		// exit;
		$this->load->view("rka/cru_kegiatan", $data);
	}
	...
	function save_kegiatan(){
		...
		if (!empty($id)) {
			$result = $this->m_rka->edit_kegiatan_skpd($data, $id, $indikator, $id_indikator_kegiatan, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn);
		}else{
			$result = $this->m_rka->add_kegiatan_skpd($data, $indikator, $satuan_target, $status_indikator, $kategori_indikator, $target, $target_thndpn);
		}
	...
	function delete_kegiatan(){
		$this->auth->restrict();
		$id = $this->input->post('id');
		$result = $this->m_rka->delete_kegiatan($id);
		if ($result) {
			$msg = array('success' => '1', 'msg' => 'Kegiatan berhasil dibuat.');
			echo json_encode($msg);
		}else{
	...
	function preview_kegiatan_renja(){
		$id = $this->input->post("id");
		$result = $this->m_rka->get_one_kegiatan(NULL, $id, TRUE);
		if (!empty($result)) {
			$data['renja'] = $result;
			$data['indikator_kegiatan'] = $this->m_rka->get_indikator_prog_keg($result->id, TRUE, TRUE);
			$data['tahun1'] = $this->m_rka->get_rka_belanja_per_tahun($id, '1');
			$data['tahun2'] = $this->m_rka->get_rka_belanja_per_tahun($id, '0');
			$this->load->view('rka/preview', $data);
		}else{
	...
	private function cetak_skpd_func($id_skpd, $ta, $id_skpd_unit){
		...
		$data2['urusan'] = $this->db->query("
			...
			INNER JOIN
				(SELECT * FROM tx_rka_prog_keg WHERE is_prog_or_keg=2 AND id_skpd > 0 AND id IN (SELECT id_prog_keg
FROM tx_rka_indikator_prog_keg WHERE target > 0)) AS keg ON keg.parent=pro.id
		...
		//$data2['program'] = $this->m_rka->get_program_skpd_4_cetak($id_skpd,$ta);
		$data['rka'] = $this->load->view('rka/cetak/program_kegiatan_preview', $data2, TRUE);
		return $data;
	}
	...
	function preview_rka(){
		...
		$skpd = $this->m_rka->get_one_rka_skpd($id_skpd_unit, TRUE);
		if (!empty($skpd)) {
			$data = $this->cetak_skpd_func($id_skpd, $ta, $id_skpd_unit);
			$this->template->load('template', 'rka/preview_rka_1', $data);
		}else{
			$this->session->set_userdata('msg_typ','err');
			$this->session->set_userdata('msg', 'Data RKA tidak tersedia.');
			redirect('home');
		}
	}
	...
	// 	$data['data_keg'] = $this->db->query('select * from tx_rka_prog_keg where id = '.$idK)->row();
	// 	$data['tahun1'] =$this->m_rka->get_rka_belanja_per_tahun221($ta, $is_tahun, $idK);

	// 	$data['keluaran'] =$this->m_rka->get_indikator_keluaran($ta, $idK);


	// 	$data['capaian'] =$this->m_rka->get_indikator_capaian($data['data_keg']->parent);
	// 	$data['nominal'] =$this->m_rka->get_nominal_rka($idK,$is_tahun);
	...
	function preview_periode_221(){
		...
		$this->load->view('rka/periode_221', $data);
	}



	function belanja_kegiatan_lihat($return, $id_kegiatan, $ta, $tahun, $is_tahun, $not=NULL){
		$result = $this->m_rka->get_kegiatan($id_kegiatan, $ta, $is_tahun, $not);
...
// print_r($data);
		$this->m_rka->add_belanja_kegiatan($data, $id_belanja);
	...
	function belanja_kegiatan_edit(){
		...
		$data['edit'] = $this->m_rka->get_one_belanja($id_belanja);
	...
	function belanja_kegiatan_hapus(){
		...
		$data['edit'] = $this->m_rka->get_one_belanja($id);

		$this->m_rka->delete_one_kegiatan($id);
	...
	private function cetak_func221($cetak=FALSE, $ta, $is_tahun, $idK, $id_skpd=NULL){
		...
		$data['keluaran'] =$this->m_rka->get_indikator_keluaran($ta, $idK);
		$data['kegiatan'] = $this->m_rka->get_kegiatan_for_221_new($ta, $idK);
		$data['capaian'] =$this->m_rka->get_indikator_capaian($data['kegiatan']->parent);
		$data['belanja'] = $this->m_rka->get_belanja_for_221_new($ta,$is_tahun, $idK);
	...
	function preview_cetak_kegiatan_for_veri($is_thn_sekarang, $idK) {
		$ta = $this->m_settings->get_tahun_anggaran();
		$keg = $this->m_rka->get_kegiatan_for_221_new($ta, $idK);
		$html = $this->cetak_func221(TRUE, $ta, $is_thn_sekarang, $idK, $keg->id_skpd);

		print_r($html);
	}

	function export_to_simda(){
		ini_set('memory_limit', '256M');
		ini_set('max_execution_time', '2000');

		$tahun = $this->m_settings->get_tahun_anggaran();
		// $id_skpd = $this->m_skpd->get_kode_unit($this->session->userdata("id_skpd"));
		$id_skpd = $this->session->userdata("id_skpd");
		$result = $this->m_rka->export_to_simda($tahun, $id_skpd);

		if ($result) {
			$this->m_rka->export_to_simda_sqlsrv($tahun, $id_skpd);
			$data['msg'] = 'Data berhasil disingkron..';
		}else{
			$data['msg'] = 'Data gagal disingkron.. Harap hubungi Administrator';
		}
	...
	function select_belanja_lihat(){
		...
		$data = $this->m_rka->get_belanja_kegiatan($id_kegiatan, $group,
	...
	function copy_belanja_kegiatan(){
		...
		$one_keg  = $this->m_rka->get_one_kegiatan(NULL, $id_keg);
		...
		$this->load->view('rka/view_copy_kegiatan', $data);
	}
	...
	function combo_copy_belanja(){
		...
		$datas = array('' => '');
		if (!empty($kd_program)) {
			$result = $this->db->query("SELECT *, tx_rka_prog_keg.id as id_keg FROM tx_rka_prog_keg
				INNER JOIN m_kegiatan
				ON tx_rka_prog_keg.kd_urusan = m_kegiatan.kd_urusan AND tx_rka_prog_keg.kd_bidang = m_kegiatan.kd_bidang AND tx_rka_prog_keg.kd_program = m_kegiatan.kd_prog AND tx_rka_prog_keg.kd_kegiatan = m_kegiatan.kd_keg
				WHERE id_skpd = '$id_skpd' AND tahun = '$tahun' AND tx_rka_prog_keg.kd_urusan = '$kd_urusan' AND tx_rka_prog_keg.kd_bidang = '$kd_bidang' AND tx_rka_prog_keg.kd_program = '$kd_program' AND tx_rka_prog_keg.id <> $id_keg
				GROUP BY tx_rka_prog_keg.kd_urusan, tx_rka_prog_keg.kd_bidang, tx_rka_prog_keg.kd_program, tx_rka_prog_keg.kd_kegiatan")->result();
			foreach ($result as $row) {
			...
			$result = $this->db->query("SELECT * FROM tx_rka_prog_keg
				INNER JOIN m_program
				ON tx_rka_prog_keg.kd_urusan = m_program.kd_urusan AND tx_rka_prog_keg.kd_bidang = m_program.kd_bidang AND tx_rka_prog_keg.kd_program = m_program.kd_prog
				WHERE id_skpd = '$id_skpd' AND tahun = '$tahun' AND tx_rka_prog_keg.kd_urusan = '$kd_urusan' AND tx_rka_prog_keg.kd_bidang = '$kd_bidang' AND tx_rka_prog_keg.id <> $id_keg
				GROUP BY tx_rka_prog_keg.kd_urusan, tx_rka_prog_keg.kd_bidang, tx_rka_prog_keg.kd_program")->result();
			foreach ($result as $row) {
			...
			$result = $this->db->query("SELECT * FROM tx_rka_prog_keg
				INNER JOIN m_bidang
				ON tx_rka_prog_keg.kd_urusan = m_bidang.kd_urusan AND tx_rka_prog_keg.kd_bidang = m_bidang.kd_bidang
				WHERE id_skpd = '$id_skpd' AND tahun = '$tahun' AND tx_rka_prog_keg.kd_urusan = '$kd_urusan' AND tx_rka_prog_keg.id <> $id_keg
				GROUP BY tx_rka_prog_keg.kd_urusan, tx_rka_prog_keg.kd_bidang")->result();
			foreach ($result as $row) {
	...
	function do_copy_belanja_kegiatan(){
		...
		$result = $this->m_rka->copy_belanja_kegiatan($keg_dari, $keg_tujuan);
