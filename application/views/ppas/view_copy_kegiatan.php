	...
	$(document).ready(function(){
		...
		$("#simpan").click(function(){
			...
			if (keg_dari == '') {
		    	...
		    	$.ajax({
					type: "POST",
					url: '<?php echo site_url("ppas/do_copy_belanja_kegiatan"); ?>',
					data: {
		...
		$(document).on("change", "#cmb_urusan", function() {
			$.ajax({
			    type: "POST",
			    url: '<?php echo site_url("ppas/combo_copy_belanja"); ?>',
...
<?php  
	$urusan = $this->db->query("SELECT * FROM `t_ppas_prog_keg` 
		INNER JOIN m_urusan 
		ON `t_ppas_prog_keg`.`kd_urusan` = m_urusan.kd_urusan
		WHERE id_skpd = '$id_skpd' AND tahun = '$tahun' AND id <> $id_keg
		GROUP BY t_ppas_prog_keg.kd_urusan")->result();
	$data_urusan = array('' => '');
...
<div style="width:900">
	<header>
		<h3>
			Copy Belanja Kegiatan PPAS Tahun <?php echo $tahun; ?>
		</h3>
