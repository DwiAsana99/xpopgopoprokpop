...
<script type="text/javascript">
	$(document).ready(function(){
		$(".cetak-kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("ppas/preview_periode_221"); ?>',
		...
		$(".tbh_kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("ppas/cru_kegiatan_skpd"); ?>',
				data: {id_program: idr},
				success: function(msg){
					if (msg!="") {
						$.facebox(msg);
		...
		$(".remove-kegiatan").click(function(){
			if (confirm('Apakah anda yakin untuk menghapus data kegiatan ini?')) {
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("ppas/delete_kegiatan"); ?>',
					data: {id: $(this).attr("idK")},
					dataType: "json",
					success: function(msg){
						if (msg.success==1) {
		...
		$("#kegiatan td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("ppas/preview_kegiatan_renja"); ?>',
	...
	<?php
		if (!empty($kegiatan)) {
			...
			foreach ($kegiatan as $row) {
				...
				$i++;
				$style='';
				$indikator_keg = $this->m_ppas->get_indikator_prog_keg($row->id);
				$id= $row->id;
				$parent = $row->parent;
