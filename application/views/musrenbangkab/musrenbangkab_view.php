<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#simpan").click(function(){
		    var valid = $("form#musrenbangkab").valid();
		    if (valid) {
                $("#jumlah_dana").val($("#jumlah_dana").autoNumeric('get'));

		    	$("form#musrenbangkab").submit();
		    };
		});
	});
</script>
<article class="module width_full">
 	<header>
		...
		<?php
			if (isset($isEdit) && $isEdit){
			    echo "Edit Data Musrenbangkab";
			} else{
			    echo "Input Data Musrenbangkab";
			}
		?>
		</h3>
 	</header>
 	<div class="module_content">
 		<form action="<?php echo $url_save_data;?>" method="POST" name="musrenbangkab" id="musrenbangkab" accept-charset="UTF-8" enctype="multipart/form-data" >
 	...
 	<footer>
		<div class="submit_link">
  			...
  			<input type="button" value="Keluar" onclick="window.location.href='<?php echo site_url('musrenbangkab'); ?>'" />
		</div>
	</footer>
</article>
