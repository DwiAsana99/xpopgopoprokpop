...
function isDate(txtDate, separator) {
	...
	$(document).ready(function(){
		...
		$('#status_ng').val($('#sumber_dana option:selected').text());
		$('#jumlah_dana').autoNumeric(numOptions);
		$('#nominaldisetujui').autoNumeric(numOptions);
		$('#nom_rekomen').autoNumeric(numOptions);
		    ...
		    var valid = $("form#usulanbansos").valid();
				...
				if (isDate(tglgabung)) {
					if (valid) {
				    	...
				    	$("#nom_rekomen").val($("#nom_rekomen").autoNumeric("get"));
...
<?php 
	if ($id_sumberdana == 1) {
		$text_hibah_bansos = 'hibah';
	}else{
		$text_hibah_bansos = 'bansos';
	}
	
 ?>
 	<div class="module_content">
 		<form action="<?php echo site_url('usulanbansos/save_evaluasi').'/'.$text_hibah_bansos;?>" method="POST" name="usulanbansos" id="usulanbansos" accept-charset="UTF-8" enctype="multipart/form-data" >
 			<input type="hidden" name="id_usulanbansos"  id='id_usulanbansos' value="<?php if(!empty($id_usulan_bansos)){echo $id_usulan_bansos;} ?>" />
 			<input type="hidden" name="status_ng" id="status_ng">
 			<table class="fcari" width="100%">
 				<tbody>
					...
					<tr>
							...
							<td><input  type="text" class="common" name="nominal_rekomendasi" id="nom_rekomen" value="<?php if(!empty($nominal_rekomendasi)){echo $nominal_rekomendasi;} ?>"/></td>
					</tr>
					<tr>
...
<!-- 					<tr>
                    	<td>Nama Pengusul</td>
                    	<td>
                        <input type="text" disabled id="nama_pengusul_ng" name="nama_pengusul_ng" class="common" value="<?php if(!empty($nama_pengusul_ng)){echo $nama_pengusul_ng;} ?>" />
                        </td>
                    </tr>
 -->
					<tr id="nama_pengusul">
