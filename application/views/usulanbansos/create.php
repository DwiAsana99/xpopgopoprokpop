...
<script type="text/javascript">
		function changesjenis(selectObj){
			...
			var dp = $('#jenis_hibah2');
			dp.empty();
			// selectedindex = "<?php echo $is_hibah_ng; ?>";
			if (selectIndex == 0){
				x.add(option1);
				x.add(option2);
				x.add(option3);
		...
		$(document).ready(function(){
			var selectIndexNG = "<?php echo $is_hibah_ng; ?>";
			var xNG = document.getElementById("jenis_hibah2");

			var option1NG = document.createElement("option");
			var option2NG = document.createElement("option");
			var option3NG = document.createElement("option");

			option1NG.text = "Uang";
			option1NG.value="1";
			option2NG.text = "Barang";
			option2NG.value = "2";
			option3NG.text = "Jasa";
			option3NG.value = "3";

			var dpNG = $('#jenis_hibah2');
			dpNG.empty();
			if (selectIndexNG == 0){
				xNG.add(option1NG);
				xNG.add(option2NG);
 	...
 	<header>
		...
		<?php
			if (isset($isEdit) && $isEdit){
			    echo "Edit Data Usulan ".$status_ng;

			} else{
			    echo "Input Data Usulan ".$status_ng;
			}
		?>
		</h3>
 	</header>
 		...
 		<form action="<?php echo site_url('usulanbansos/save');?>" method="POST" name="usulanbansos" id="usulanbansos" accept-charset="UTF-8" enctype="multipart/form-data" >
 					...
 					<tr id="sumber_dana">
                    	...
                    	<td>
                    		<select id="sumber_dana" name="sumber_dana" class="common" onchange="changesjenis(this);">
                    			<?php
                    				if($id_sumberdana==1 || $is_hibah_ng == 1){
                    			?>
                    			...
                    			?>
                    			<?php
                    				if($id_sumberdana==2 || $is_hibah_ng == 0){
                    			?>
          ...
          <tr>
          		...
          		<!-- <tr>
          			<td>Nama Pengusul</td>
          			<td>
          				<input type="text" name="nama_pengusul_ng" class="common" value="<?php echo(!empty($nama_pengusul_ng))?$nama_pengusul_ng:''; ?>">
          			</td>
          		</tr> -->
