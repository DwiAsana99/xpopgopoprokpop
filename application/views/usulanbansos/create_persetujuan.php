		...
		$("#simpan").click(function(){
			...
			if(idfile != '' || iduserfile !='' ){
				if (valid) {
			    	...
			    	$("#nominaldisetujui").val($("#nominaldisetujui").autoNumeric('get'));
...
<article class="module width_full">
 	...
 	<div class="module_content">
 		<form action="<?php echo site_url('usulanbansos/save_persetujuan');?>" method="POST" name="usulanbansos" id="usulanbansos" accept-charset="UTF-8" enctype="multipart/form-data" >
 			...
 			<table class="fcari" width="100%">
					...
					<tr>
					    ...
					    <td>
					        <select name="status" class="common">
					         	<option value="1" <?php if($id_keputusan == 1){?> selected <?php } ?> >Disetujui</option>
		                        <option value="0" <?php if($id_keputusan == 0){?> selected <?php } ?> >Tidak Disetujui</option>
            		        </select>
					    </td>
					</tr>
					<tr>
					    <td>Nominal</td>
			            <td>
							<input type="text" name="nominaldisetujui" class="common" id="nominaldisetujui" value="<?php if(!empty($nominal_setuju)){echo $nominal_setuju;} ?>">
			            </td>
		            </tr>
		            <tr>
			            <td>Keterangan</td>
					    <td>
							<input type="text" name="keterangan" id="keterangan" value="<?php echo $alasan ?>">
					    </td>
					 </tr>
              ...
              <div id="autocomplete_element_desa" class="autocomplete_element"></div>
					...
					<tr>
						<td>Satuan</td>
						<td><input class="common" disabled type="text" name="satuan" id="satuan"value="<?php if(!empty($satuan)){echo $satuan;} ?>"/></td>
					</tr>
					<tr>
						<td>Nilai Usulan</td>
