...
<script type="text/javascript">
	$(document).ready(function(){
		$(".tbh_kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("renja_perubahan/cru_kegiatan_skpd"); ?>',
				data: {id_program: idr},
		...
		$(".remove-kegiatan").click(function(){
			if (confirm('Apakah anda yakin untuk menghapus data kegiatan ini?')) {
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("renja_perubahan/delete_kegiatan"); ?>',
		...
		$("#kegiatan td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("renja_perubahan/preview_kegiatan_renja"); ?>',
	...
	<?php
		if (!empty($kegiatan)) {
			...
			foreach ($kegiatan as $row) {
				$i++;
				$style='';
				$indikator_keg = $this->m_renja_trx_perubahan->get_indikator_prog_keg_perubahan($row->id);
				$id= $row->id;
				$parent = $row->parent;
				if($row->id_renja != NULL){
