<script type="text/javascript">
	...
	$("#tambah_indikator_kegiatan").click(function(){
		...
		$("#indikator_box_kegiatan texarea").attr("name", name);
...
<div style="width: 1300px">
	...
	?>
    <br />
		<form action="<?php echo site_url('renja_perubahan/save_kegiatan');?>" method="POST" name="kegiatan" id="kegiatan" accept-charset="UTF-8" enctype="multipart/form-data" >
			...
			<input type="hidden" name="id_renja" value="<?php if(!empty($kegiatan->id_renja)){echo $kegiatan->id_renja;} ?>" />
			<input type="hidden" name="id_renstra" value="<?php if(!empty($detil_kegiatan_1[0]->id_renstra)){echo $detil_kegiatan_1[0]->id_renstra;} ?>" />
			...
			<table class="fcari" width="100%">
				<tbody>
                    ...
                    <tr>
                      ...
                      <tr>
                      	<th align="center" width="50%">DPA Tahun <?php echo $ta;?></th>
                        <th align="center" width="50%">Perubahan Tahun <?php echo $ta;?></th>
                      </tr>
                      </table>
                      </div>
                      </td>
							...
							?>
								...
								<div style="width: 100%;">
									<table class="table-common" width="100%">
										...
										<tr style="width:100%">
											<td>Target</td>
                                            <td><input style="width: 100%;" type="text" class="target" name="target[<?php echo $i; ?>]" value="<?php echo (!empty($row->target))?$row->target:''; ?>" readonly></td>
					...
					<tr>
						<td>&nbsp;&nbsp;Keterangan Perubahan</td>
							<td colspan="2">
								<textarea class="common" name="keterangan" style="font-family:Arial, Helvetica, sans-serif" rows="5"><?php echo (!empty($kegiatan->keterangan))?$kegiatan->keterangan:''; ?></textarea>
								<br />&nbsp;&nbsp; *Apabila melakukan perubahan pada data renja ini, harap mengisi keterangan perubahan pada form diatas.
							</td>
					</tr>
					<tr>
						<td>&nbsp;&nbsp;Pagu Indikatif</td>
						<td>Rp. <input type="text" id ="nominal" name="nominal" value="<?php if(!empty($kegiatan->nominal)){echo $kegiatan->nominal;}else{echo '0';} ?>" readonly/>
                        </td>
						<td bgcolor="#CCCCCC">Rp. <input type="text" id="nominal_thndpn" name="nominal_thndpn" value="<?php if(!empty($kegiatan->nominal_thndpn)){echo $kegiatan->nominal_thndpn;}else{echo '0';} ?>" readonly/>
                        </td>
					</tr>
					<tr>
						<td>&nbsp;&nbsp;Lokasi</td>
		            ...
		            <ul class="nav nav-tabs">
		              <li class="active"><a href="#tahun1" data-toggle="tab">DPA Tahun <?php echo $ta;?></a></li>
		              <li ><a href="#tahun2" data-toggle="tab">Perubahan Tahun <?php echo $ta;?></a></li>
		            </ul>
							              ...
							              <?php echo form_dropdown('sumberdana_2', $sumber_dana, NULL, 'data-placeholder="Pilih Sumber Dana" class="common chosen-select" id="sumberdana_2" name="sumberdana_2"'); ?>
											...
											<tbody id="list_tahun_2">
												...
												<tr id="<?php echo $gIndex_2 ?>">
													...
													<?php 
													if ($row->id_status_renja == 1) {
													?>
														<td colspan="2">
															<span id='ubahrowng' class='icon-pencil' onclick='ubahrowng(<?php echo $row->id ?>, 2)' style='cursor:pointer' title='Ubah Belanja'></span>
														</td>
														<?php
													}else{
														?>
														...
														<td>
															<span id='hapusrowng' class='icon-remove' onclick='hapusrowng(<?php echo $row->id ?>, 2)' style='cursor:pointer' title='Hapus Belanja'></span>
														</td>
													<?php
													}
													?>
  ...
  function jenis_belanjanya_2(p_nama, p_jenis) {
    $.ajax({
      ...
      success: function(msg){
        ...
        // prepare_chosen();
        $("#"+p_nama).attr('disabled', true).trigger("liszt:updated");
      }
    });
  }
...
// var status = true;
		...
		if (status) {
			$.ajax({
			    type: "POST",
			    url: '<?php echo site_url("renja_perubahan/belanja_kegiatan_save"); ?>',
			    dataType: 'html',
			    data: { 
			    ...
			    success: function(msg){
			    	...
			    	if (id_belanja == '') {
			    		$.ajax({
							type: "POST",
							url: '<?php echo site_url("renja_perubahan/belanja_keg_lihat_ajax"); ?>',
							dataType: 'html',
							data: {
								id_kegiatan : id_kegiatan,
								is_tahun_sekarang : is_tahun+1,
				    			tahun : tahun_skr
							},
							success: function(msg2){
								$('#list_tahun_1').html(msg2);
			    				$('#id_belanja_1').val('');
							}
						});
			    	}
	...
	function clear_belanja(clue, tahun){
	    ...
	    $("#cb_jenis_belanja_"+tahun).attr('disabled', false).trigger("liszt:updated");
	    $("#cb_kategori_belanja_"+tahun).attr('disabled', false).trigger("liszt:updated");
	    $("#cb_subkategori_belanja_"+tahun).attr('disabled', false).trigger("liszt:updated");
	    $("#cb_belanja_"+tahun).attr('disabled', false).trigger("liszt:updated");
	    $("#sumberdana_"+tahun).attr('disabled', false).trigger("liszt:updated");
	    prepare_chosen();
	    $('#uraian_'+tahun).prop('readonly', false);
	    $('#det_uraian_'+tahun).prop('readonly', false);
	}
  ...
  function ubahrowng(id_belanja, tahun){
    ...
    if (check == '' || check == null) {
      ...
      $.ajax({
          type: "POST",
          url: '<?php echo site_url("renja_perubahan/belanja_kegiatan_edit"); ?>',
          dataType: 'json',
          data: {
          id_kegiatan : id_kegiatan,
          id_belanja : id_belanja,
          ...
          success: function(msg){
            ...
            for (var i = 0; i < msg.list.length; i++) {
              ...
              if (msg.list[i].id_status_renja == 1) {
              	row += "<td colspan='2'><span id='ubahrowng' class='icon-pencil' onclick='ubahrowng("+msg.list[i].id+","+tahun+")' style='cursor:pointer' title='Ubah Belanja'></span></td>";
              }else{
            ...
            if (tahun == 1) {
	            ...
	            // $('#cb_jenis_belanja_2').disabled = true;
	            // $("#cb_jenis_belanja_2").trigger("chosen:updated");
	            // $("#cb_jenis_belanja_2").attr('disabled', true).trigger("chosen:updated");
	            $('#uraian_2').prop('readonly', true);
	            $('#det_uraian_2').prop('readonly', true);
        	}
  ...
  function hapusrowng(id_belanja, tahun){
    ...
    $.ajax({
          type: "POST",
          url: '<?php echo site_url("renja_perubahan/belanja_kegiatan_hapus"); ?>',
          dataType: 'json',
          data: {
          id_kegiatan : id_kegiatan,
          id_belanja : id_belanja,
