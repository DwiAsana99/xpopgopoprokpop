...
<article class="module width_full" style="width: 100%;">
	<header>
	  <h3>Verifikasi CIK Perubahan</h3>
	</header>
    ...
    <table id="tabel_cik" class="table-common" width="99.9%">
        ...
        <tbody id="preview_cik">
				...
				foreach($urusan as $row_urusan){
						$bidang = $this->m_cik_perubahan->get_data_bidang_cik($row_urusan->kd_urusan,$id_skpd,$bulan);
					foreach($bidang as $row_bidang){
						$program = $this->m_cik_perubahan->get_data_program_cik($row_urusan->kd_urusan,$row_bidang->kd_bidang,$id_skpd,$bulan);
					foreach($program as $row)
					{
						$kegiatan = $this->m_cik_perubahan->get_data_kegiatan_cik($row->id_skpd,$bulan,$row_urusan->kd_urusan,$row_bidang->kd_bidang,$row->kd_program);
				...
				?>
                	<tr>
						...
						?>
									<a href="javascript:void(0)" class="<?php echo ($row_kegiatan->is_prog_or_keg==1)?'icon-th-large veri_prog':'icon-list veri_keg';?>" id-r="<?php echo $row_kegiatan->id; ?>" id-b="<?php echo $bulan;?>"></a>
						<?php
