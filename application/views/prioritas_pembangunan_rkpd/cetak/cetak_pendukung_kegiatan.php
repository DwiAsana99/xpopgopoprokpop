				...
				foreach ($sasaran as $key_sasaran => $row_sasaran) {
					...
					foreach ($program as $key_program => $row_program) {
						...
						foreach ($kegiatan as $key_kegiatan => $row_kegiatan) {
							$prog_keg = $this->m_prioritas_pembangunan_rkpd->get_prog_keg_by_prioritas($row_sasaran->id, 1)->result();
					...
					<table border="1" style="border-collapse: collapse; margin: -2px -2px -1px -2px;" width="101.5%">					
						<?php
							foreach ($prog_keg as $row_prog_keg) {
								...
								$keg_ng = $this->db->query('SELECT * FROM t_renja_prog_keg WHERE parent = '.$row_prog_keg->id)->result();
								foreach ($keg_ng as $row_keg_ng) {
									echo "<tr>
											<td>".$row_keg_ng->kd_urusan."</td>
											<td>".$row_keg_ng->kd_bidang."</td>
											<td>".$row_keg_ng->kd_program."</td>
											<td>".$row_keg_ng->kd_kegiatan."</td>
											<td>".$row_keg_ng->nama_prog_or_keg."</td>
										</tr>";
								}
							// 	if ($parent != $row_prog_keg->id) {
							// 		$parent = $row_prog_keg->id;
							// 		$pro = $this->db->query('SELECT * FROM t_renja_prog_keg WHERE parent = '.$parent)->result();
							// 		echo "<tr>
							// 				<td>".$pro->kd_urusan."</td>
							// 				<td>".$pro->kd_bidang."</td>
							// 				<td>".$pro->kd_program."</td>
							// 				<td>".$pro->kd_kegiatan."</td>
