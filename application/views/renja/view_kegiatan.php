...
<script type="text/javascript">
	$(document).ready(function(){
		$(".cetak-kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("renja/preview_periode_221"); ?>',
		...
		$(".tbh_kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("renja/cru_kegiatan_skpd"); ?>',
				data: {id_program: idr},
				success: function(msg){
					if (msg!="") {
						$.facebox(msg);
		...
		$(".remove-kegiatan").click(function(){
			if (confirm('Apakah anda yakin untuk menghapus data kegiatan ini?')) {
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("renja/delete_kegiatan"); ?>',
		...
		$("#kegiatan td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("renja/preview_kegiatan_renja"); ?>',
	...
	<?php
		if (!empty($kegiatan)) {
			...
			foreach ($kegiatan as $row) {
				$i++;
				$style='';
				$indikator_keg = $this->m_renja_trx->get_indikator_prog_keg($row->id);
				$id= $row->id;
				$parent = $row->parent;
				if($row->id_renstra != NULL && $row->id_renstra != 0){
