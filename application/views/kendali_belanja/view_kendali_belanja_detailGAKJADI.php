...
<table>
	...
	<?php
		for($i=1;$i<=12;$i++){
			...
			$sql1 = "select * from tx_dpa_rencana_aksi where id_dpa_prog_keg = '".$id_dpa_prog_keg."' and bulan = '".$i."'";
			$count_output = $this->db->query($sql1)->num_rows();
			$detail_output = $this->db->query($sql1)->result();

			$bobot_kom_prog = $this->db->query('SELECT SUM(bobot) AS bobot FROM tx_dpa_rencana_aksi WHERE id_dpa_prog_keg = "'.$id_dpa_prog_keg.'" AND bulan<='.$i)->row()->bobot;

			foreach ($detail_output as $key => $row) {
				$target_kom_prog = $this->db->query('SELECT SUM(target) AS target FROM tx_dpa_rencana_aksi WHERE id_dpa_prog_keg = "'.$id_dpa_prog_keg.'" AND bulan<='.$i.' AND aksi = "'.$row->aksi.'"')->target;
				if ($key == 0) {
	?>
		<tr>
			<td rowspan="<?php echo $num_row_span ?>">BLN <?php echo $i; ?></td>
			<td><?php echo $row->aksi; ?></td>
			<td rowspan="<?php echo $num_row_span; ?>"><?php echo $bobot_kom_prog; ?></td>
			<td><?php echo $target_kom_prog; ?></td>
			<td><?php echo 'ss'; ?></td>
			<td><?php echo 'ss'; ?></td>
			<td align="center" rowspan="<?php echo $num_row_span ?>">
				<?php if ($data_triwulan->status_kendali != 3): ?>			
					<a href="javascript:void(0)" idP="<?php echo $id_dpa_prog_keg;?>" idT = "<?php echo $i?>" class="icon-pencil edit-kinerja-triwulan" title="Kendali Bulanan <?php echo $i;?>"></a>
				<?php else: ?>
					-
				<?php endif ?>
			</td>
