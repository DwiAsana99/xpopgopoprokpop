...
<script type="text/javascript">
	$(document).ready(function(){
		if ($('#jenis').val() === 'cetak3') {
			$('#CetakEXCEL').attr('style', 'display : inline !important;');
			$('#CetakPDF').attr('style', 'display : inline !important;');
			$('#CetakLaporan').attr('style', 'display : none !important;');
		}else{
			$('#CetakEXCEL').attr('style', 'display : none !important;');
			$('#CetakPDF').attr('style', 'display : none !important;');
			$('#CetakLaporan').attr('style', 'display : inline !important;');
		}
		...
		$(document).on("click", "#CetakPDF", function(){
			...
			var jenis=$('#jenis').val();
			if (jenis=='cetak3') {
				link = '<?php echo site_url("rpjmd/do_cetak_indikasi_program_prioritas/1"); ?>';
			}
			$.blockUI({
				message: 'Cetak dokumen sedang di proses, mohon ditunggu hingga file terunduh secara otomatis ...',
				css: window._css,
		...
		$(document).on("click", "#CetakEXCEL", function(){
			...
			var jenis=$('#jenis').val();
			if (jenis=='cetak3') {
				link = '<?php echo site_url("rpjmd/do_cetak_indikasi_program_prioritas/2"); ?>';
			}
			$.blockUI({
				message: 'Cetak dokumen sedang di proses, mohon ditunggu hingga file terunduh secara otomatis ...',
				css: window._css,
...
<article class="module width_full" style="width: 138%; margin-left: -19%;">
 	...
 	<footer>
 		<div class="submit_link">
 			<input id="CetakEXCEL" type="button" value="Cetak Excel">
 			<input id="CetakPDF" type="button" value="Cetak PDF">
