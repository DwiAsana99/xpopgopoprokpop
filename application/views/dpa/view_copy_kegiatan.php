	...
	$(document).ready(function(){
		...
		$("#simpan").click(function(){
			...
			if (keg_dari == '') {
		    	...
		    	$.ajax({
					type: "POST",
					url: '<?php echo site_url("dpa/do_copy_belanja_kegiatan"); ?>',
					data: {
		...
		$(document).on("change", "#cmb_urusan", function() {
			$.ajax({
			    type: "POST",
			    url: '<?php echo site_url("dpa/combo_copy_belanja"); ?>',
...
<?php
	$urusan = $this->db->query("SELECT * FROM tx_dpa_prog_keg pk
...
<div style="width:900">
	<header>
		<h3>
			Copy Belanja Kegiatan DPA Tahun <?php echo $tahun; ?>
		</h3>
