	...
	<tbody>
			...
			foreach ($aksi as $key => $row) {
				...
				foreach ($rencana_aksi as $key2 => $row2) {
					 ...
					 foreach ($aksi_per_bulan as $key3 => $row3) {
					 	...
					 	if ($row3->target > 0) {
					 		$for_capaian = formatting::currency(($row3->capaian/$row3->target)*100, 2);
					 	}
		...
		?>
			<tr>
				...
				<?php } ?>
					...
					<td valign="top"><?php echo number_format($bobot_kom_prog, 2, ',', '.'); ?></td>
					...
					<td valign="top"><?php echo formatting::currency($for_capaian*$bobot_kom_prog, 2); ?></td>
		...
		<?php
			...
			foreach ($keg as $key_keg => $row_keg) { 
				...
				foreach ($rencana_aksi_keg as $key_keg2 => $row_keg2) {
					 ...
					 $progres_fisik = $this->db->query('SELECT SUM(capaian/target*100)/COUNT(capaian) AS progres 
					 ...
					 foreach ($aksi_per_bulan_keg as $key_keg3 => $row_keg3) {
					 	...
					 	if ($sum_anggaran_aksi_keg->sum_ang > 0) {
					 		// $for_serapan_keg = number_format(($realisasi_keg->realisasi/$sum_anggaran_aksi_keg->sum_ang)*100, 2, ',', '.');
		...
		?>
			<tr>
				...
				<?php if ($key_keg3 == 0){ ?>
					<td valign="top"  rowspan="<?php echo $count_per_bulan_keg; ?>"><?php echo number_format($bobot_kom_keg, 2, ',', '.'); ?></td>
				<?php } ?>
				...
				<?php if ($key_keg3 == 0) { ?>
					<td valign="top"  rowspan="<?php echo $count_per_bulan_keg; ?>"><?php echo formatting::currency($progres_fisik*$bobot_kom_keg/100, 2); ?></td>
